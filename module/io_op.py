...
class IO():
    def __init__(self):
        if platform == "Windows":
            ...
            self.slash = '/'
        self.Outputdir = ''.join([os.getcwd(),self.slash,"output",self.slash])
    ...
    def plotDataWrite(self,FileName,xvalue,yvalue):
        PlotDataName = FileName.replace(".lvm",'_plotdata.txt').split("/")[-1].split('\\')[-1]
        path = self.Outputdir +'%s'%PlotDataName
